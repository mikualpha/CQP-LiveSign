...
namespace Native.Csharp.App.Event
{
	public class Event_GroupMessage
	{
		...
		public void ReceiveGroupMessage(object sender, GroupMessageEventArgs e)
		{
            ...
            //若发送用户及群组不在列表，则直接跳出
            if (!DouyuCheck.getInstance().isAdmin(e.FromQQ.ToString()) || !DouyuCheck.getInstance().isGroup(e.FromGroup.ToString()))
            {
                e.Handled = false;
                return;
            }

            if (e.Msg.Contains("/斗鱼订阅-"))
            {
                int index = e.Msg.IndexOf('-');
                string room = e.Msg.Substring(index + 1);
                DouyuCheck.getInstance().SubscribeByGroup(e.FromGroup.ToString(), room);
                EnApi.Instance.SendGroupMessage(e.FromGroup, "订阅主播[" + DouyuCheck.getInstance().getOwner(room) + "]成功!");
                e.Handled = true;
                return;
            }

            if (e.Msg.Contains("/斗鱼取消订阅-"))
            {
                int index = e.Msg.IndexOf('-');
                string room = e.Msg.Substring(index + 1);
                DouyuCheck.getInstance().Desubscribe(e.FromGroup.ToString(), room, 1);
                EnApi.Instance.SendGroupMessage(e.FromGroup, "取消订阅主播[" + DouyuCheck.getInstance().getOwner(room) + "]成功!");
                e.Handled = true;
                return;
            }
